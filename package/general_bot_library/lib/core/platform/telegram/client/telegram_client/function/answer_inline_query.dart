...
extension AnswerInlineQueryDataOn on GeneralBotPlatformTelegramCoreBaseLibrary {
  /// method AnswerInlineQuery
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        if (result_data is Map) {
          ...
          result_data.forEach((key, value) {
            ...
            if (value is Map) {
              if (key == "reply_markup") {
                jsonData[key] = GeneralUniverseUtils.replyMarkupTgApiToTdlib(
                  replyMarkup: value,
                );
              }
              if (key == "input_message_content") {
                jsonData[key] = utils_inputMessageContentInlineQueryTgToTdlib(
