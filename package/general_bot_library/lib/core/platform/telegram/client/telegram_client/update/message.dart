...
/// method MessageData
extension MessageDataDataOn on GeneralBotPlatformTelegramCoreBaseLibrary {
  ...
  Future<Map> message_Message({
    ...
    if (message["message_thread_id"] is num && message["message_thread_id"] > 0) {
      if (message["is_topic_message"] == true) {
        ...
        message_thread_json["message_thread_id"] = GeneralUniverseUtils.messageTdlibToApi(
    ...
    if (generalBotClientTelegramLibraryData.is_bot == false) {
    ...
    final Map msg = <dynamic, dynamic>{
      "@type": "message",
      "id": message["id"],
      "message_id": GeneralUniverseUtils.messageTdlibToApi(message["id"]),
      ...message_thread_json,
      ...message_from_json,
    ...
    if (message["reply_markup"] is Map) {
      final Map? reply_markup = GeneralUniverseUtils.replyMarkupTdlibToTgApi(
  ...
  FutureOr<Map?> message_toJson({
    ...
    if (update["@type"] == "updateNewMessage") {
      ...
      if ((message["is_outgoing"] == true)) {
        if (generalBotClientTelegramLibraryData.is_bot) {
          return null;
        }
      }
      bool isSkipUpdate() {
