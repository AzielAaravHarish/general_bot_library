...
import 'package:general_bot_library/schemes/schemes.dart';
...
Future<void> generateSchema({
  ...
  for (var i = 0; i < general_bot_library_database_schemes.length; i++) {
    // general_bot_library_database_schemes[i].general_universe_extension_updateForce(data: data);
    general_bot_library_database_schemes[i].general_universe_extension_updateForce(data: {
      "from_app_id": "",
      "owner_account_user_id": 0,
    });
  }
  await jsonToScripts(
    general_bot_library_database_schemes,
    directory: Directory(
      path.join(
...
//       .trim());
  ...
  // }
  try {
    for (var i = 0; i < general_bot_library_respond_schemes.length; i++) {
      general_bot_library_respond_schemes[i].general_universe_extension_updateForce(data: {
        "@extra": "",
        "@expire_date": "",
        "@client_id": "",
      });
    ...
    await jsonToScripts(
      general_bot_library_respond_schemes,
      directory: Directory(
        path.join(
          directoryBase.uri.toFilePath(),
          "respond_scheme",
  ...
  try {
    for (var i = 0; i < general_bot_library_schemes.length; i++) {
...
Future<void> generateIsarDatabase() async {
  ...
  final Directory directory = Directory(path.join(
    ...
    "..",
    "general_bot_library_database_universe_scheme",
  ));
  ...
  for (var i = 0; i < general_bot_library_database_schemes.length; i++) {
    ...
    Map<String, dynamic> data = general_bot_library_database_schemes[i];
